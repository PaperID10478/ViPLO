    ...
    def forward(self, x: torch.Tensor, box_coords=None, box_segs=None, pose_attention_weight=None, need_patch=False):
        ...
        if box_coords is not None :
            if box_coords[0].shape[1] == 4 or box_coords[0].shape[1] == 17:
                ...
                #assert torch.sum(cls_tok!=res['global_x']) == 0

                #detection_attn_mask = res['detection_attn_mask'][0::self.heads].squeeze(1).exp()[:, 1:]
                #detection_attn_mask = detection_attn_mask / detection_attn_mask.sum(dim=1, keepdim=True)
                #spatial_tok = detection_attn_mask.unsqueeze(-1) * self.positional_embedding.to(x.dtype)[1:, :].detach()
                #spatial_tok = spatial_tok.sum(dim=1)
                ...
                #center_point = fine_det_center_point(box_coords, self.input_resolution, self.patch_size)

                #center_tok = self.positional_embedding.to(x.dtype)[center_point]
                #mask_cls_tok = mask_cls_tok @ self.proj
                #cls_tok = cls_tok @ self.proj
                #mask_cls_tok = mask_cls_tok / mask_cls_tok.norm(dim=-1, keepdim=True)
                #cls_tok = cls_tok / cls_tok.norm(dim=-1, keepdim=True)
                


                #return mask_cls_tok, cls_tok, spatial_tok, center_tok


            ### DEFR case    
            elif box_coords[0].shape[1] == 8:
                res = self.transformer(x, self.patch_size, box_coords)
                mask_cls_tok = res['x']
                mask_cls_tok = self.ln_post(mask_cls_tok[0])
                mask_cls_tok = mask_cls_tok @ self.proj
