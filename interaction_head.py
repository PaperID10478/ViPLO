...
from ops import compute_spatial_encodings, binary_focal_loss, generate_pose_heatmap, make_pose_box, compute_spatial_encodings_with_pose, compute_spatial_encodings_with_pose_to_attention
from pose_models import Pose2d, BottleNeck, BasicBlock
...
class InteractionHead(Module):
    ...
    def preprocess(self,
        ...
        for b_idx, detection in enumerate(detections):
            ...
            if self.pose:
                ...
                pose_heatmap = detection['pose_heatmap']
            ...
            if append_gt:
                ...
                if self.pose:
                    ...
                    pose_heatmap = torch.cat([target['pose_heatmap'], pose_heatmap])
                    assert human_joints.shape[0] == human_joints_score.shape[0]
                    assert human_joints.shape[0] == pose_heatmap.shape[0]
            ...
            if self.pose:
                results.append(dict(
                    ...
                    pose_heatmap=pose_heatmap[active_human_idx],
                ))
            else:
                results.append(dict(
                    boxes=boxes[active_idx].view(-1, 4), 
    ...
    def forward(self,
        ...
        box_segs = None
        if self.pose:
            pose_heatmaps = [detection['pose_heatmap'] for detection in detections]
            ...
            pose_heatmaps = [None for _ in detections]
        ...
        if self.pose_cls:
            ...
            box_pair_labels, box_pair_prior = self.box_pair_head(
                ...
                box_coords, box_labels, box_scores, pose_heatmaps, human_joints, human_joints_score, targets, backbone=self.backbone, images=images, pose_box_coords=pose_box_coords
            )
        else:
            box_pair_features, box_pair_local_features, boxes_h, boxes_o, object_class,\
            box_pair_labels, box_pair_prior = self.box_pair_head(
                features, image_shapes, box_features, pose_box_features,
                box_coords, box_labels, box_scores, pose_heatmaps, human_joints, human_joints_score, targets
            )

        box_pair_features = torch.cat(box_pair_features)
        if self.local_pose:
            #box_pair_local_features = torch.cat(box_pair_local_features)
            #logits_p = self.box_pair_predictor(torch.cat([box_pair_features, box_pair_local_features], dim=1))
...
class MessageMBF(MultiBranchFusion):
    ...
    # def _forward_object_local_pose_nodes(self, local_feature: Tensor, spatial: Tensor):
    #     n_h, n = spatial.shape[:2]
    #     assert local_feature.shape[0] == n_h
    #     assert local_feature.shape[1] == n
    #     #print("doing local pose")
...
class GraphHead(Module):
    ...
    def __init__(self,
        ...
        if self.backbone_name == "CLIP_CLS":
            ...
            if self.local_pose and not self.pose_cls:
        ...
        if self.local_pose:
            self.obj_to_sub = MessageMBF(
                ...
                representation_size, node_type= 'object_local_pose', #'object_local_pose',
                cardinality=16
            )
        else:
            self.obj_to_sub = MessageMBF(
                node_encoding_size, 1024,
                representation_size, node_type= 'object', #'object_local_pose',
                cardinality=16
            )
        ...
        #if self.pose:
            #self.concat_pose_spatial_head = nn.Sequential(nn.Linear(1536, 1024), nn.ReLU())
            #self.pose_network = Pose2d(BottleNeck, [4,6,3])
            #if self.local_pose:
            #    self.pose_attention = nn.Sequential(nn.Linear(1024, 1024), nn.ReLU(), nn.Linear(1024, 17), nn.Softmax(dim=-1))
    ...
    def forward(self,
        ...
        box_labels: List[Tensor], box_scores: List[Tensor], pose_heatmaps : List[Tensor], human_joints: List[Tensor], human_joints_score: List[Tensor],
        ...
        for b_idx, (coords, labels, scores, pose_heatmap, human_joint, human_joint_score) in enumerate(zip(box_coords, box_labels, box_scores, pose_heatmaps, human_joints, human_joints_score)):
            n = num_boxes[b_idx]
            device = box_features.device
            
            n_h = torch.sum(labels == self.human_idx).item()
            if self.pose:
                assert pose_heatmap.shape[0] == n_h
            ...
            #box_pair_spatial = compute_spatial_encodings_with_pose([coords[x]], [coords[y]], [human_joint.repeat_interleave(n, dim=0)], [image_shapes[b_idx]])

            #box_pair_spatial_semantic = self.spatial_head(box_pair_spatial)
            ...
            if self.pose:
                if self.local_pose:
                    #pose_attention_weight = self.pose_attention(box_pair_spatial_semantic) * human_joint_score.repeat_interleave(n, dim=0)
                    ...
                    if self.pose_cls:
                        pose_local_feature, _ = backbone.encode_image(images[b_idx].unsqueeze(0), [pose_box_coords[b_idx].reshape(-1, 17, 4)], None, pose_attention_weight, need_patch=False)
                        pose_local_feature = self.box_head(pose_local_feature)
                    else:
            ...
            #if self.local_pose:
            #   all_box_pair_local_features.append(pose_local_feature_reshaped[x_keep, y_keep])
